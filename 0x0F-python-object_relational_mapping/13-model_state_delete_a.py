#!/usr/bin/python3
"""SQLAlchemy ORM script to delete states containing the letter 'a'.

This script connects to a MySQL database using SQLAlchemy ORM, ensures that
the schema exists, and deletes all states whose names contain the letter 'a'
from the `states` table. The changes are committed to the database.
...
if __name__ == "__main__":
    ...
    with sess() as s:
        # Delete all states whose names contain the letter 'a'.
        # `synchronize_session=False` ensures SQLAlchemy does not attempt
        # to update objects in memory, improving efficiency for bulk deletes.
        s.query(State).filter(State.name.like('%a%')).delete(
